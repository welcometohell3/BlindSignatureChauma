...
public:
    ...
    // }
    // static Num generatePrime(size_t n_bits)
    // {
    //     // Проверяем входные аргументы
    //     if (n_bits == 0)
    //     {
    //         throw std::invalid_argument("Number of bits must be greater than 0");
    //     }

    //     // Генерируем случайное число с n_bits бит
    //     Num candidate = Num::random_bits(n_bits, func);

    //     // Если число нечетное, делаем его нечетным
    ...
    //     {
    //         // Увеличиваем число на 2, чтобы оно оставалось нечетным
    //         candidate += 2;
    //     }
    ...
    void print(
        ...
        word base = 10,
        const char *alphabet = "0123456789abcdefghijklmnopqrstuvwxyz") const
    {
        if (size() == 0)
        {
            text.push_back('0');
